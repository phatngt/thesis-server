...
export class UserService extends BaseCRUD {
  ...
  async createUser(data: UserCreateDTO) {
    ...
    try {
      const role = await this.roleUserRepository.findOneBy({
        name: UserPermision.USER,
      });

      if (!role) throw new HttpException('Role user not existed', 500);

      const type = await this.userTypeRepository.findOneBy({
  ...
  async update(id: number, data: UserUpdateDTO, user?: User) {
    try {
      ...
      const { password, ...result } = await this.updateById(
        id,
        auditData,
        user,
      );
