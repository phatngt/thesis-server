...
import { Light, PlantTypes } from 'src/models/plant-type.entity';
import { Plants } from 'src/models/plants.entity';
import { ImageType } from 'src/types/file';
import { Plant, PlantType } from 'src/types/plant';
...
export class PlantService extends BaseCRUD {
  ...
  async createPlantType(plantType: PlantType, admin: User): Promise<PlantTypes> {
    try {
      ...
      const newPType = await this.plantTypeRepository.save({
        ...
        light: plantType.light ?? Light.MEDIUM,
        ...auditAdd(admin)
      })
      return newPType;
    } catch (error) {
  ...
  async createPlant(plantData: Plant, user: User) {
    try {
      // Get plant type
      // const pType = await this.plantTypeRepository.findOneBy({ id: plantData.plant_type_id });
